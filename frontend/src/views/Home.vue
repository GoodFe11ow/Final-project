...
const isCreateTaskDialogOpen = computed({
  get: () => route.query.modal === 'create',
  set: (nextOpen) => {
    if (!nextOpen) {
  ...
  <AppShell>
    <section class="flex flex-1 flex-col gap-5 pb-2">
      <div class="space-y-3 pt-1 text-center">
        <div class="text-[2.05rem] font-semibold uppercase tracking-[0.2em] text-slate-900">
          Home
        </div>
        <p class="text-[0.72rem] font-semibold uppercase tracking-[0.24em] text-slate-400">
          {{ todayLabel }}
        </p>
        <h2
      ...
      <Card
        ...
        <CardContent class="p-0">
          <div class="flex items-center justify-between px-5 py-4">
            <p class="text-[0.72rem] font-semibold uppercase tracking-[0.22em] text-slate-500">
              Current Tasks
            </p>
          ...
          <div v-if="currentTasks.length > 0" class="divide-y divide-slate-100">
            <article v-for="task in currentTasks" :key="task.id" class="flex items-center gap-3 px-5 py-4">
              ...
              <span class="rounded-full px-3 py-1 text-sm font-semibold shadow-[inset_0_1px_0_rgba(255,255,255,0.85)]"
      ...
      <Card class="rounded-[2rem] border-slate-200/80 bg-white shadow-[0_24px_60px_-32px_rgba(15,23,42,0.26)]">
        <CardContent class="space-y-4 p-5">
          <Button type="button"
            ...
            <span class="mr-2 flex size-7 items-center justify-center rounded-full bg-white text-blue-500 shadow-sm">
          ...
          <div class="grid grid-cols-2 gap-3">
            <Button v-for="action in quickActions" :key="action.label" type="button" variant="ghost"
    ...
    <TaskEditorDialog :open="isCreateTaskDialogOpen" mode="create" :task="null"
      @update:open="isCreateTaskDialogOpen = $event" @saved="tasksStore.fetchTasks()" />
  </AppShell>
</template>
